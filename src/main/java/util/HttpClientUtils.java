...
public class HttpClientUtils {
     ...
     * 默认客户端级别的设置
     */
    private RequestConfig defaultRequestConfig = RequestConfig.custom().setRedirectsEnabled(this.isHandleRedirects())  //自动处理重定向
            .setMaxRedirects(this.getMaxRedirects())        //自动重定向次数
            .setConnectionRequestTimeout(DEFAULT_CONNECTION_REQUEST_TIMEOUT).setConnectTimeout(DEFAULT_CONNECT_TIMEOUT).setSocketTimeout(DEFAULT_REQUEST_TIMEOUT).build();
    /**
     * 默认链接相关的设置
     */
    private ConnectionConfig defaultConnectionConfig = ConnectionConfig.custom().setBufferSize(this.getSocketBufferSize()).setCharset(Charset.forName(DEFAULT_ENCODING)).build();
    /**
     * 默认Socket相关的设置
     */
    private SocketConfig defaultSocketConfig = SocketConfig.custom().setTcpNoDelay(this.isTcpNodelay()).setSoTimeout(DEFAULT_REQUEST_TIMEOUT).setSoLinger(this.getSoLinger()).setSoReuseAddress(this.isSoReuseaddr()).setSoKeepAlive(this.isKeepAlive()).build();
    ...
    public HttpClientUtils() {
         ...
         * 创建链接管理器
         */
        Registry<ConnectionSocketFactory> socketFactoryRegistry = RegistryBuilder.<ConnectionSocketFactory>create().register("https", SSLConnectionSocketFactory.getSocketFactory()).register("http", PlainConnectionSocketFactory.getSocketFactory()).build();
         ...
         * 创建httpClient
         */
        closeableHttpClient = HttpClients.custom().setRetryHandler(new HttpRequestRetryHandler())                 //设置请求重试Handler
                .setKeepAliveStrategy(new HttpConnectionKeepAliveStrategy())    //设置keep-alive连接保持活动的策略
                .setUserAgent(this.getUserAgent()).setDefaultRequestConfig(defaultRequestConfig).setConnectionManager(poolingHttpClientConnectionManager).build();
    ...
    public final String httpGet(String url, Map<String, String> queryParams, int requestTimeout, String encoding) {
        ...
        try {
            ...
            if (requestTimeout > 0) {
                 ...
                 */
                RequestConfig requestConfig = RequestConfig.copy(defaultRequestConfig).setSocketTimeout(requestTimeout).build();
...
//            }
    ...
    public String httpGetAddHeader(String url, Map<String, String> headerParams) {
        return httpGetAddHeader(url, headerParams, DEFAULT_REQUEST_TIMEOUT, DEFAULT_ENCODING);
    }
     ...
     * @throws RuntimeException
     */
    public final String httpGetAddHeader(String url, Map<String, String> headerParam, int requestTimeout, String encoding) {
        Preconditions.checkArgument(!Strings.isNullOrEmpty(url), "Param url must be not null and empty");
        Preconditions.checkArgument(!Strings.isNullOrEmpty(encoding), "Param encoding must be not null and empty");
         ...
         */
        long start = System.currentTimeMillis();
        StringBuilder sb = new StringBuilder("consumer_log - headerParams ");
        sb.append(urlToSend).append("|");
        if (headerParam != null) {
            sb.append(headerParam.toString()).append("|");
        }
        ...
        try {
            ...
            if (headerParam != null && !headerParam.isEmpty()) {
                for (Map.Entry entry : headerParam.entrySet()) {
                    httpGet.addHeader(String.valueOf(entry.getKey()), String.valueOf(entry.getValue()));
                }
            }
...
//            }
    ...
    public final String httpPutForm(String url, Map<String, String> putParams) {
        return httpPutForm(url, putParams, DEFAULT_REQUEST_TIMEOUT, DEFAULT_ENCODING);
    }

    public final String httpPutForm(String url, Map<String, String> putParams, int requestTimeout) {
        return httpPutForm(url, putParams, requestTimeout, DEFAULT_ENCODING);
    }
    ...
    public final String httpPutForm(String url, Map<String, String> putParams, int requestTimeout, String encoding) {
        Preconditions.checkArgument(!Strings.isNullOrEmpty(url), "Param url must be not null and empty");
        Preconditions.checkArgument(putParams != null && !putParams.isEmpty(), "Param putParams must be not null and empty");
        Preconditions.checkArgument(!Strings.isNullOrEmpty(encoding), "Param encoding must be not null and empty");
        ...
        sb.append(urlToSend).append("|");
        // 参数打印控制1000字符以内
        sb.append(putParams.toString()).append("|");
...
//            }
            ...
            sb.append("Exception:").append(ex.getMessage()).append("|");
            throw new RuntimeException("HttpClientUtils.httpPutForm error.", ex);
        } finally {
            // 结果打印控制1000字符以内
            sb.append(result).append("|");
            sb.append(System.currentTimeMillis() - start);
    ...
    public final String httpPutJson(String url, String jsonParams, int requestTimeout) {
        return httpPutJson(url, jsonParams, requestTimeout, DEFAULT_ENCODING);
    }
...
//            }
            ...
            sb.append("Exception:" + ex.getMessage()).append("|");
            throw new RuntimeException("HttpClientUtils.httpPutJson error.", ex);
        } finally {
            // 返回结果打印控制1000字符以内
            sb.append(result).append("|");
            sb.append(System.currentTimeMillis() - start);
